...
class BiLSTM(nn.Module):
    ...
    def __init__(self, vob_size, embed_size, hidden_size, num_layers, batchsize, max_len):
        super(BiLSTM, self).__init__()
        drp = 0.1
        self.batch_size = batchsize
        self.max_len = max_len
        self.embed_size = embed_size
        self.embedding = nn.Embedding(vob_size, embed_size) ## nn.embeding like a table for converting vocabulary to embedding
    ...
    def forward(self, x, seq_lengths):
        # x [bz,max_len]
        h_embedding = self.embedding(x) #20864,200,10 / [bz,max_len,embed_size]
        h_embedding = nn.utils.rnn.pack_padded_sequence(h_embedding, seq_lengths.cpu().numpy(), batch_first=True)
        h_bilstm, _ = self.bilstm(h_embedding) #[bz,max_len,2*hidden]
        h_bilstm, _ = nn.utils.rnn.pad_packed_sequence(h_bilstm, batch_first=True, padding_value=0.,total_length=x.shape[1])
        out = self.linear(h_bilstm.data) # [bz, max_len, 8]
    ...
    def init_embeddings(self):
        ...
        self.embedding.weight.data[0] = torch.zeros(self.embed_size)
...
class BiLSTM2(nn.Module):
  ...
  def forward(self, sentence,dropout=True):
  ...
  def init_embeddings(self, word_pad_idx):
    # initialize embedding for padding as zero
    self.embedding.weight.data[word_pad_idx:word_pad_idx+2] = torch.zeros(self.embedding_dim)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self,vocab_size,emb_size,hidden_size,out_size):
        ...
        self.bilstm = BiLSTM2(vocab_size,emb_size,hidden_size,out_size)
